        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mx-auto'>
          {notifications.map((due) => (
            <Link key={due.id} href={`/dashboard/orders/${due.id}`}>
              <div className='border px-6 py-4 rounded-xl flex flex-col items-center bg-red-600/10'>
                <CldImage
                  width='80'
                  height='80'
                  crop='thumb'
                ...
                />
                <p className='font-medium text-center'>{due.name}</p>
                <p className='text-sm text-gray-500'>{due.tel}</p>
                ...
                <div className='text-red-600 font-medium text-center'>
                  {today === due.due_date && (
                    <p>This job is due for collection today</p>
                  )}
